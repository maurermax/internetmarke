    ...
    return this._getSoapClient()
      .then(client => {
        ...
        order.positions.forEach(position => {
          wrappedPositions.push({'v3:position':position});
        });

        return client[method]({
          userToken: this._user.getToken(),
          pageFormatId: this._defaultPageFormatId,
          positions: wrappedPositions,
          total: order.total,
          createManifest: order.createManifest,
          createShoppingList: order.createShoppingList
        });
      })
      .then(response => {
