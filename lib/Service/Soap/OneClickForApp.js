...
const SoapService = require('./Soap'),
  User = require('../../User'),
  { WSDL, OUTPUT_FORMATS, LAYOUT_ZONES } = require('../../constants');
...
class OneClickForAppService extends SoapService {
  ...
  async authenticateUser(user) {
    ...
    this._pageFormats = (await client.retrievePageFormatsAsync())[0].pageFormat;
    ...
    return this._getSoapClient()
      .then(client => {
        ...
        if (outputFormat === OUTPUT_FORMATS.PDF) {
          obj.pageFormatId = _.find(this._pageFormats, { name: 'DIN A4 Normalpapier', isAddressPossible:order.positions[0].voucherLayout === LAYOUT_ZONES.ADDRESS, isImagePossible: false}).id;
        }
        order = Object.assign(obj, order);
        if (outputFormat === OUTPUT_FORMATS.PDF) {
          order.positions[0].position = {
            labelX: 1,
            labelY: 1,
            page: 1
          }
