...
class OneClickForAppService extends SoapService {
  ...
  async authenticateUser(user) {
    ...
    this._defaultPageFormatId = _.find(pageFormats[0].pageFormat, { name: 'DIN A4 Normalpapier', isAddressPossible: false, isImagePossible: false}).id;
    ...
    return this._getSoapClient()
      .then(client => {
        ...
        if (outputFormat === OUTPUT_FORMATS.PDF) {
          obj.pageFormatId = this._defaultPageFormatId;
        }
